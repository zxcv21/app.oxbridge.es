...
function p_01003_mostrar_actividad(tipo,d,a,m){
	...
	/*cambiar color borde del botÃ³n*/
	...
	switch(tipo){
		...
		case 'tar':{
			...
			/*solo devuelve string con html de vocabulario del dÃ­a para imprimir*/
			if(m!=undefined)
				return html_target;
		...
		if(p_01003_hijo.tagName=="H2"){
			...
			if(p_01003_contador_divs%4==0){
				...
				if(p_01003_altura_contenido>p_01003_altura_pagina){
					...
					if(p_01003_hijos[i-1].tagName=="H2"){
						p_01003_altura_contenido= p_01003_hijos[i-1].offsetHeight;
						p_01003_printWindowBody.insertBefore(p_01003_salto_pagina, p_01003_printWindowBody.childNodes[i-1]);
					}
					else{
						p_01003_printWindowBody.insertBefore(p_01003_salto_pagina, p_01003_printWindowBody.childNodes[i]);
